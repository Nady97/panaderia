...
<div class="dashboard-container">
    <x-card class="mb-4">
        <div class="p-4">
            <h2 class="fw-bold mb-3 text-main">
                <i class="bi bi-pencil-circle me-2 text-gold"></i> Editar Factura
            </h2>
            <p class="text-muted">Factura: <strong>{{ $facturaInterna->nro_factura }}</strong></p>
        </div>
    </x-card>
    ...
    <form action="{{ route('facturas_internas.update', $facturaInterna) }}" method="POST" class="needs-validation">
        @csrf
        @method('PUT')
            ...
            <div class="p-3">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control @error('cliente_ci') is-invalid @enderror" 
                               id="cliente_ci" name="cliente_ci" value="{{ old('cliente_ci', $facturaInterna->cliente_ci) }}" required>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control @error('cliente_telefono') is-invalid @enderror" 
                               id="cliente_telefono" name="cliente_telefono" value="{{ old('cliente_telefono', $facturaInterna->cliente_telefono) }}">
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('cliente_direccion') is-invalid @enderror" 
                              id="cliente_direccion" name="cliente_direccion" rows="2">{{ old('cliente_direccion', $facturaInterna->cliente_direccion) }}</textarea>
            ...
            <div class="p-3">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" class="form-control @error('fecha_emision') is-invalid @enderror" 
                               id="fecha_emision" name="fecha_emision" value="{{ old('fecha_emision', $facturaInterna->fecha_emision->format('Y-m-d')) }}" required>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="number" class="form-control @error('puntos_ganados') is-invalid @enderror" 
                               id="puntos_ganados" name="puntos_ganados" value="{{ old('puntos_ganados', $facturaInterna->puntos_ganados) }}" min="0">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('total') is-invalid @enderror" 
                           id="total" name="total" value="{{ old('total', $facturaInterna->total) }}" step="0.01" min="0.01" required readonly>
            ...
            <div class="p-3">
                ...
                <div id="productos-container">
                    @foreach($facturaInterna->detalles as $detalle)
                        <div class="row mb-3 p-2 border rounded bg-light" data-producto-index="{{ $loop->index }}">
                            <div class="col-md-5">
                                <small class="text-muted">Producto</small>
                                <div class="fw-semibold">{{ $detalle->producto->nombre }}</div>
                                <input type="hidden" name="productos[{{ $loop->index }}][producto_id]" value="{{ $detalle->producto->id }}">
                            </div>
                            <div class="col-md-2">
                                <small class="text-muted">Cantidad</small>
                                <div>{{ $detalle->cantidad }}</div>
                                <input type="hidden" name="productos[{{ $loop->index }}][cantidad]" value="{{ $detalle->cantidad }}">
                            </div>
                            <div class="col-md-2">
                                <small class="text-muted">Precio Unit.</small>
                                <div>Bs. {{ number_format($detalle->precio_unitario, 2) }}</div>
                                <input type="hidden" name="productos[{{ $loop->index }}][precio_unitario]" value="{{ $detalle->precio_unitario }}">
                            </div>
                            <div class="col-md-2">
                                <small class="text-muted">Desc./Total</small>
                                <div>Bs. {{ number_format($detalle->descuento, 2) }} / {{ number_format($detalle->total_linea, 2) }}</div>
                                <input type="hidden" name="productos[{{ $loop->index }}][descuento]" value="{{ $detalle->descuento }}">
                            </div>
                            <div class="col-md-1 text-end">
                                <button type="button" class="btn btn-sm btn-danger btn-eliminar-producto" data-index="{{ $loop->index }}">
        ...
        <x-card>
            <div class="p-3 d-flex gap-2 justify-content-end">
                <a href="{{ route('facturas_internas.show', $facturaInterna) }}" class="btn btn-secondary">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let productoIndex = document.querySelectorAll('[data-producto-index]').length;
    ...
    // Botones eliminar para productos existentes
    document.querySelectorAll('.btn-eliminar-producto').forEach(btn => {
