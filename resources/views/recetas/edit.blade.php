...
<div class="space-y-5">
            ...
            <div>
                <div class="flex items-center gap-2 mb-1">
                    <h2 class="text-xl font-bold text-[var(--text-primary)] flex items-center gap-2">
                        <i class="bi bi-journal-check text-[var(--gold-dark)]"></i> Editar Receta
                    </h2>
                    <x-badge type="warning" class="px-3">Actualizacion</x-badge>
                </div>
                <p class="text-sm text-[var(--text-muted)]">Ajusta ingredientes para {{ $receta->producto->nombre ?? 'producto' }}</p>
            </div>
    ...
    <x-card>
        <div class="p-4 md:p-6 space-y-6">
            ...
            <form action="{{ route('recetas.update', $receta->id) }}" method="POST" id="formEditarReceta" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <div class="flex items-stretch rounded-xl border border-[var(--border-color)] bg-[var(--bg-input)]">
                            ...
                            <input type="number" name="rendimiento" class="w-full bg-transparent px-3 py-2 text-sm text-[var(--text-primary)] outline-none" value="{{ old('rendimiento', $receta->rendimiento) }}">
                        </div>
                        @error('rendimiento')
                            <p class="mt-1 text-xs text-red-600"><i class="bi bi-exclamation-circle mr-1"></i>{{ $message }}</p>
                        @enderror
                    ...
                    @foreach($receta->insumos as $index => $insumo)
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 ingrediente-item">
                                ...
                                <div class="flex items-stretch rounded-xl border border-[var(--border-color)] bg-[var(--bg-input)]">
                                    ...
                                    <select name="insumos[]" class="w-full bg-transparent px-3 py-2 text-sm text-[var(--text-primary)] outline-none" required>
                                        ...
                                        @foreach($insumos as $insumoOption)
                                            <option value="{{ $insumoOption->id }}" {{ (old('insumos.' . $index, $insumo->id) == $insumoOption->id) ? 'selected' : '' }}>{{ $insumoOption->nombre }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div>
                                ...
                                <div class="flex items-stretch rounded-xl border border-[var(--border-color)] bg-[var(--bg-input)]">
                                    <span class="flex items-center px-3 text-[var(--text-muted)]"><i class="bi bi-123"></i></span>
                                    <input type="number" name="cantidades[]" class="w-full bg-transparent px-3 py-2 text-sm text-[var(--text-primary)] outline-none" step="0.01" value="{{ old('cantidades.' . $index, $insumo->pivot->cantidad) }}" required>
                                </div>
                            </div>
                            <div>
                                ...
                                <div class="flex items-stretch rounded-xl border border-[var(--border-color)] bg-[var(--bg-input)]">
                                    ...
                                    <select name="unidades[]" class="w-full bg-transparent px-3 py-2 text-sm text-[var(--text-primary)] outline-none" required>
                                        ...
                                        @foreach($unidades as $unidad)
                                            <option value="{{ $unidad }}" {{ (old('unidades.' . $index, $insumo->pivot->unidad) == $unidad) ? 'selected' : '' }}>{{ $unidad }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                ...
                <div class="rounded-xl border border-[var(--border-color)] bg-[var(--bg-input)] p-4">
                    <div class="flex items-center justify-between">
                        ...
                        <input class="h-5 w-5 accent-[var(--btn-bg)]" type="checkbox" id="activa" name="activa" value="1" {{ old('activa', $receta->activa) ? 'checked' : '' }}>
                    </div>
                ...
                <div class="flex justify-end gap-3 border-t border-[var(--border-color)] pt-4">
                    ...
                    <button type="submit" class="inline-flex items-center gap-2 rounded-xl bg-yellow-500 px-4 py-2 text-sm font-semibold text-white hover:bg-yellow-600">
                        <i class="bi bi-save"></i> Actualizar Receta
                    </button>
                </div>
            </form>
        </div>
